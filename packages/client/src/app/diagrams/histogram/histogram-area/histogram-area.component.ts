  ...
  implements AfterViewInit, OnChanges
{
  ...
  override set data(data: T[]) {
    super.data = data
    const max = Math.max(...data.reduce((u, i) => [...u, ...i.values.map((v) => v || 0), 0], <number[]>[]))
  ...
  protected override createScales() {
    this.margin = {
      ...this.margin,
      left: this.getYLabelsMaxLength() * HistogramAreaComponent.ASSUMED_CHAR_WIDTH + 8,
    }
    super.createScales()
  }
  ...
  protected drawAreas() {
    ...
    const data = this.areaFunctions.map((areaFn, ix) => ({
      area: <string>(
        areaFn(this.spanGaps && this.spanGaps[ix] ? this.data.filter((e) => isDefined(e.values[ix])) : this.data)
      ),
      color:
