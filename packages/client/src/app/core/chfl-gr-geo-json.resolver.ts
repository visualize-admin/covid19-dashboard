...
export class ChflGreaterRegionsGeoJsonResolver implements Resolve<ExtendedGeoFeatureCollection> {
  ...
  resolve(): Promise<ExtendedGeoFeatureCollection> {
    if (!this.fileRequest) {
      this.fileRequest = firstValueFrom(
        this.httpClient.get<ExtendedGeoFeatureCollection>('/assets/geojson/chfl_gr.feature.json'),
      )
    }
