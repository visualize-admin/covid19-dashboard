...
@Component({
  selector: 'bag-detail-card-vacc-persons-indication',
  templateUrl: './detail-card-vacc-persons-indication.component.html',
  styleUrls: ['./detail-card-vacc-persons-indication.component.scss'],
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class DetailCardVaccPersonsIndicationComponent
  extends BaseDetailCardVaccinationComponent<EpidemiologicVaccPersonsIndicationDevelopmentData>
  implements OnInit
{
  ...
  keys: Record<'info' | 'descriptionTitle' | 'metaRel' | 'metaAbs' | 'tooltipRel' | 'tooltipAbs' | 'legendZero', string>
    ...
    const ctx: TooltipListContentData = {
      ...
      entries: data.values
        .map((val, ix): TooltipListContentEntry & { _val: number | null } => ({
          ...
          value: adminFormatNum(val, isRel ? 2 : undefined, isRel ? '%' : ''),
          type: 'square',
          _val: val,
        }))
  ...
  protected init() {
    this.keys = {
      info: 'Vaccination.VaccPersons.Card.Indication.Info',
      descriptionTitle: 'Vaccination.VaccPersonsFull.DetailTitle',
      metaRel: 'Vaccination.VaccPersons.Card.Indication.Weekly.Legend.Rel',
      metaAbs: 'Vaccination.VaccPersons.Card.Indication.Weekly.Legend.Abs',
      tooltipRel: 'Vaccination.VaccPersons.Card.Indication.Weekly.Tooltip.Rel',
      tooltipAbs: 'Vaccination.VaccPersons.Card.Indication.Weekly.Tooltip.Abs',
      legendZero: 'Vaccination.VaccPersons.Card.Indication.Weekly.Legend.Zero',
    }
  }
